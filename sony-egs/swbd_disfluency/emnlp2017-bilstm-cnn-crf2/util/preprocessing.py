...
def perpareDataset(embeddingsPath, datasets, frequencyThresholdUnknownTokens=50, reducePretrainedEmbeddings=False, valTransformations=None, padOneTokenSentence=True):
    ...
    for datasetName, dataset in datasets.items():
        ...
        trainDataProsody = 'data/%s/train_prosody_feats.pkl' % datasetName 
        devDataProsody = 'data/%s/dev_prosody_feats.pkl' % datasetName 
        testDataProsody = 'data/%s/test_prosody_feats.pkl' % datasetName 
        paths = [trainData, devData, testData, trainDataProsody, devDataProsody, testDataProsody]
...
def readEmbeddings(embeddingsPath, datasetFiles, frequencyThresholdUnknownTokens, reducePretrainedEmbeddings):
    ...
    embeddingsIn = gzip.open(embeddingsPath, "rt") if embeddingsPath.endswith('.gz') else open(embeddingsPath,
                                                                                               encoding="utf8")
#    embeddingsIn = codecs.getreader('utf-8')(gzip.open(embeddingsPath, "rt"), errors='replace') if embeddingsPath.endswith('.gz') else open(embeddingsPath,encoding="utf8")
#    embeddingsIn = gzip.open(embeddingsPath, "rt") if embeddingsPath.endswith('.gz') else open(embeddingsPath,
#                                                                                               encoding="utf8")
    ...
    for line in embeddingsIn:
        ...
        word = split[0].encode('utf-8')
        #word = split[0].decode('utf-8')

        if embeddingsDimension == None:
#            embeddingsDimension = len(split) - 0
...
#                word2Idx[word] = len(word2Idx)
                try:
                    # test if word is ascii, except otherwise
...
#                    print ('skipping non-ASCII word %s' % word)
                    pass
...
def addProsodicInformation(sentences, prosody):
    """Reads in pkl file with prosody vectors"""
    for sentenceIdx in range(len(sentences)):
        #sentences[sentenceIdx]['prosody'] = []
        utt_id = sentences[sentenceIdx]['utt_id'][0]
        if len(sentences[sentenceIdx]['tokens']) == 1:
            # empty numpy array with shape (422,3) for padding token
            padding_prosody = np.zeros((prosody[utt_id][0].shape[0],prosody[utt_id][0].shape[1]))
            prosody[utt_id] = np.array([prosody[utt_id][0],padding_prosody])
            #max_word_len = len(prosody[utt_id][0])
            #num_feats = len(prosody[utt_id][0][0])
            #padding_prosody = [ [0.0]*num_feats for _ in range(max_word_len) ]
            #prosody[utt_id].append(padding_prosody)
        sentences[sentenceIdx]['prosody'] = prosody[utt_id]
...
def createMatrices(sentences, mappings, padOneTokenSentence):
    ...
    for sentence in sentences:
        ...
        if 'pause' in sentence:
            row['pause'] = list(np.array(sentence['pause'], dtype='float32'))
        if 'word_dur_phone' in sentence:
            row['word_dur_phone'] = list(np.array(sentence['word_dur_phone'], dtype='float32'))
        if 'word_dur_word' in sentence:
            row['word_dur_word'] = list(np.array(sentence['word_dur_word'], dtype='float32'))

        if len(row['tokens']) == 1 and padOneTokenSentence:
            paddedSentences += 1
            if 'pause' in sentence:
                row['pause'].append(0.0)
            if 'word_dur_phone' in sentence:
                row['word_dur_phone'].append(0.0)
            if 'word_dur_word' in sentence:
                row['word_dur_word'].append(0.0)
...
def createPklFiles(datasetFiles, mappings, cols, commentSymbol, valTransformation, padOneTokenSentence):
    ...
    try:
        pr_train = open(datasetFiles[3], 'rb')
        prosody_train = pkl.load(pr_train, encoding='iso-8859-1')
        addProsodicInformation(trainSentences,prosody_train)
        pr_dev = open(datasetFiles[4], 'rb')
        prosody_dev = pkl.load(pr_dev, encoding='iso-8859-1')
        addProsodicInformation(devSentences,prosody_dev)
        pr_test = open(datasetFiles[5], 'rb')
        prosody_test = pkl.load(pr_test, encoding='iso-8859-1')
        addProsodicInformation(testSentences,prosody_test)
    except:
        pass
...
def extendMappings(mappings, sentences):
    ...
    try:
        sentenceKeys.remove('pause')
    except:
        pass
    try:
        sentenceKeys.remove('pause_before')
    except:
        pass
    try:
        sentenceKeys.remove('pause_after')
    except:
        pass
    try:
        sentenceKeys.remove('word_dur_phone')
    except:
        pass
    try:
        sentenceKeys.remove('word_dur_word')
    except:
        pass
